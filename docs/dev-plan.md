...
- [X] `[A]` 实现任务类型自动识别：根据输入自动判断是代码漏洞检测还是恶意分析
- [X] `[A]` 实现分析结果聚合：从多步工具调用结果中提取关键发现
- [X] `[A]` 实现置信度计算：根据工具结果一致性和 LLM 自身判断计算置信度
- [X] `[A]` 实现标准分析流程 fallback（当 Agent 自主决策失败时使用）
- [X] `[A]` Agent 命令行 Demo 2.0：上传恶意脚本 → 自动分析 → 输出判定+依据
- [X] `[A]` 用 5 个样本测试 Agent，记录准确率和 Token 消耗

### 3.2 Prompt 调优（成员 A，第3-5天，贯穿后续）

- [X] `[A]` 准备 3 个标准测试样本（已知漏洞代码、正常代码、恶意脚本）
- [X] `[A]` 对每个样本测试当前 Prompt 效果，记录问题
- [X] `[A]` 调整 System Prompt：补充输出格式示例（Few-shot）
- [X] `[A]` 调整工具描述文案：使 LLM 更准确地选择工具
- [X] `[A]` 添加约束规则：禁止跳过工具直接下结论、必须引用证据
- [X] `[A]` 对比调整前后的分析质量，记录改进

### 3.3 文件特征提取工具（成员 B，第1-3天）

- [X] `[B]` 安装 pefile：`pip install pefile`
- [X] `[B]` 实现 PE 文件分析：导入表提取、节区信息、编译时间戳
- [X] `[B]` 实现文件字符串提取：提取所有可打印字符串（≥4 字符）
- [X] `[B]` 实现 Office 文档分析：检测宏、提取 VBA 代码
- [X] `[B]` 实现文件类型自动识别（基于 magic bytes，不依赖扩展名）
- [X] `[B]` 实现 `extract_file_features` 工具入口函数：返回文件基本信息+特征
- [X] `[B]` 写测试：上传 benign.exe → 应返回正确的导入表信息
...
- [X] `[C]` 实现对话管理模块：存储/加载对话历史、上下文窗口管理
- [X] `[C]` 实现 AI 报告摘要生成：调用 LLM 将分析结果凝练为摘要
- [X] `[C]` 实现分析结果智能排序：按危险等级/置信度排序
- [X] `[C]` 实现文件上传安全控制：类型白名单校验、大小限制（50MB）、文件 Hash 记录
- [X] `[C]` 实现临时文件清理：分析完成后自动删除临时文件
- [X] `[C]` API 全部可用验证：用 Postman 或 curl 逐个测试所有端点

### 4.2 任务队列管理（成员 C，第2-3天）

- [X] `[C]` 方案选择：如果 Redis 可用则用 Celery，否则用 Python `threading` 实现简易异步队列
- [X] `[C]` 实现任务状态机：pending → analyzing → done / failed
- [X] `[C]` 实现并发控制：同时最多处理 3 个分析任务
- [X] `[C]` API `GET /api/tasks/{task_id}` 实时返回当前状态和已完成步骤

### 4.3 前端项目搭建（成员 D，第1-2天）

- [X] `[D]` 确认 React 项目结构：pages / components / hooks / services / utils
- [X] `[D]` 配置 React Router：`/`、`/submit`、`/analysis/:taskId`、`/report/:taskId`、`/history`
- [ ] `[D]` 配置 Ant Design 全局主题
- [X] `[D]` 封装 API 请求模块：`frontend/src/services/api.ts`（axios 实例+拦截器）
- [X] `[D]` 封装 WebSocket 连接 Hook：`useWebSocket(taskId)` → 自动连接/重连/返回最新状态

### 4.4 仪表盘首页（成员 D，第2-3天）

- [X] `[D]` 实现首页统计卡片：今日任务数、总任务数、高危发现数、平均耗时
- [X] `[D]` 实现最近分析列表：展示最近 5 个任务（类型+状态+时间）
- [X] `[D]` 实现快捷操作入口：代码扫描、恶意分析两个大按钮
- [X] `[D]` 首页数据全部通过 API 从后端实时获取

### 4.5 任务提交页面（成员 D，第3-4天）

- [X] `[D]` 实现"代码漏洞检测"Tab：代码编辑区（带语法高亮）、语言选择下拉框、粘贴/上传切换
- [X] `[D]` 实现"恶意代码分析"Tab：拖拽上传区域、文件信息预览、支持多文件类型
- [X] `[D]` 实现提交按钮 + 防重复提交 + 提交成功跳转到分析页面
- [X] `[D]` 实现文件上传进度条 + 上传完成反馈

### 4.6 分析过程页 — 思维链展示（成员 D，第4-5天）

- [X] `[D]` 设计"思维链卡片"组件：Thought（💭蓝色）、Action（🔧橙色）、Observation（👁绿色）、Done（✅）、Error（❌红色）
- [X] `[D]` 实现卡片渐入动画：每张卡片从底部滑入（CSS transition + 延时）
- [X] `[D]` 实现正在执行的步骤：脉动动画（pulse）+ "分析中..."文字
- [X] `[D]` 实现步骤展开/折叠：点击可查看工具调用的详细参数和返回结果
- [X] `[D]` 实现分析进度条：显示已完成步骤/总步骤（预估）
- [X] `[D]` 实现分析完成 → 自动跳转报告页，或手动点击"查看报告"按钮
...
- [X] `[D]` 实现风险雷达图组件：多维度安全评分（代码质量、认证、授权、数据保护、加密、日志）
- [X] `[D]` 实现漏洞类型分布饼图/环形图（Recharts）
- [X] `[D]` 实现 ATT&CK 热力图组件：用 Recharts Heatmap 展示战术×技术矩阵
- [X] `[D]` 实现漏洞严重等级柱状图：高危/中危/低危/信息
- [X] `[D]` 图表支持响应式、支持导出为图片、配色与主题一致

### 5.3 交互式对话面板（成员 D，第3天）

- [X] `[D]` 实现分析报告页底部对话面板：气泡式聊天界面
- [X] `[D]` 实现追问输入框 + 发送按钮，支持 Shift+Enter 换行
- [X] `[D]` 实现 Agent 回复气泡：打字动画效果 + Markdown 渲染
- [X] `[D]` 实现对话历史显示（按时间排序）
- [X] `[D]` 对话内容通过 API 发送到后端，后端调用 Agent 并返回回复

### 5.4 历史任务管理页（成员 D，第4天）

- [X] `[D]` 实现任务列表页：Ant Design Table + 分页
- [X] `[D]` 实现搜索框：按任务类型/状态/时间范围筛选
- [X] `[D]` 实现每行操作按钮：查看报告、删除任务
- [X] `[D]` 实现删除确认弹窗（Ant Design Modal）

### 5.5 前后端联调（全员，第4-5天）

- [X] `[全员]` 代码漏洞检测全流程联调：提交代码 → Agent 分析 → 思维链展示 → 报告展示 → PDF 导出
- [X] `[全员]` 恶意代码分析全流程联调：上传文件 → Agent 分析 → 结果展示 → IOC 清单 → PDF 导出
- [X] `[C]+[D]` 验证 WebSocket 实时推送：思维链各步骤能逐条展示、无丢失、无乱序
- [X] `[C]+[D]` 验证错误处理：文件上传超大 → 提示限制、API 超时 → 友好提示、网络断开 → 重连
- [X] `[C]+[D]` 验证对话追问功能：发送问题 → 显示回复 → 多轮对话正常

### 5.6 测试样本准备（成员 B，第3-5天）

- [X] `[B]` 准备 5 个漏洞代码样本（Python 3个 + Java 1个 + JavaScript 1个）
- [X] `[B]` 包含漏洞类型：SQL注入、XSS、命令注入、路径遍历、硬编码密钥
- [X] `[B]` 准备 5 个恶意代码样本（PE 2个 + 脚本 2个 + Office宏 1个）
- [X] `[B]` 样本需安全无害（仅用于演示，不含真实恶意功能）
- [X] `[B]` 为每个样本编写"预期分析结果"文档，用于对比验证
- [X] `[B]` 准备 3 个正常样本（对照：确认不会误报）
